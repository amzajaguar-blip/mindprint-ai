...
async function setServerSession(access_token: string) {
...
export default function Login() {
  ...
  const verifyOtp = async (e: React.FormEvent) => {
    ...
    await setServerSession(data.session.access_token);
    setLoading(false);
    navigate("/dashboard");
  };
  ...
  return (
    <div className="min-h-screen bg-[#08080F] flex items-center justify-center px-4">
      ...
      <div className="relative z-10 w-full max-w-sm">
        ...
        <div className="border border-[#8B5CF6]/25 rounded-2xl p-7 bg-black/50 space-y-4">
          ...
          {step === "email" && (
            <>
              <button
                ...
                className="w-full py-3 rounded-xl font-semibold text-white flex items-center justify-center gap-3 border border-gray-700 hover:border-gray-500 hover:bg-white/5 transition-all disabled:opacity-50"
              >
                {googleLoading ? (
              ...
              <div className="flex items-center gap-3">
                ...
                <span className="text-gray-600 text-xs">oppure con email</span>
                <div className="flex-1 h-px bg-gray-800" />
              </div>
            </>
          )}
          ...
          {step === "email" ? (
            <form onSubmit={sendOtp} className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl bg-[#111] border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-[#8B5CF6]/60 transition-colors text-sm"
                />
              </div>
            ...
            <form onSubmit={verifyOtp} className="space-y-4">
              <div>
                <label className="text-gray-400 text-xs uppercase tracking-widest mb-2 block">Codice OTP (8 cifre)</label>
                <input
                  type="text"
                  value={otp}
                  ...
                  autoFocus
                  className="w-full px-4 py-3 rounded-xl bg-[#111] border border-gray-800 text-white placeholder-gray-600 focus:outline-none focus:border-[#8B5CF6]/60 transition-colors text-sm tracking-[0.4em] text-center"
                />
              </div>
              {error && <p className="text-red-400 text-xs">{error}</p>}
              <button
